...
/** Surface with built-in padding, for consistent card density across pages. */
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn("rounded-lg border border-border bg-surface/80 p-5 backdrop-blur-sm", className)}
      {...props}
    />
  );
}
